// Atmospheric Nitrogen Deposition - Interactive Features
...
function initScrollAnimations() {
  ...
  const sections = document.querySelectorAll('.transport-section, .soil-section, .mycorrhizal-section, .competition-section, .case-studies-section, .solutions-section, .cta-section');
  ...
  const counterObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const isRate = text.includes('kg/ha/yr');
        const isNumber = /^\d+$/.test(text.replace(/,/g, '').replace(/\+/g, ''));

        if (isNumber || isPercentage || isMultiplier || isRate) {
...
function animateCounter(element, finalText) {
  ...
  const isRate = finalText.includes('kg/ha/yr');
  ...
  const timer = setInterval(() => {
    ...
    else if (isRate) displayText += ' kg/ha/yr';
...
function initCardHoverEffects() {
  const cards = document.querySelectorAll('.transport-card, .soil-card, .network-card, .competition-card, .solution-card');
...
style.textContent = `
  ...
  .transport-section, .soil-section, .mycorrhizal-section, .competition-section,
